...
MonteCarlo_MTSastra::MonteCarlo_MTSastra(int my_id, int root_proc, int num_procs, int num_beads,
                               int elec_beads,double mass, int num_states,double beta_num_beads, 
...
void MonteCarlo_MTSastra::gen_initx(){
    for (int bead=0; bead<elec_beads; bead++) {
        for (int state=0; state<num_states; state++) {
            x(bead,state) = elec_dist(myRand.doub());
        }
    }
}
...
void MonteCarlo_MTSastra::gen_initp(){
    for (int bead=0; bead<elec_beads; bead++) {
        for (int state=0; state<num_states; state++) {
            p(bead,state) = elec_dist(myRand.doub());
        }
    }
}
