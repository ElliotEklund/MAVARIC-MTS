...
/*
 ...
 Else, return 0.
 */
inline int check_sys_in(std::string root, std::vector<double> & params){
    ...
    /* Check Mass */
    if(!is_positive(params[0],fileName,"Mass")){
        result = -1;
    }
    ...
    /* Check Temperature */
    if(!is_positive(params[2],fileName,"Temperature")){
        result = -1;
    }

    /* Check MC Step Size */
    if(!is_positive(params[3],fileName,"MC Step Size")){
        result = -1;
    }
