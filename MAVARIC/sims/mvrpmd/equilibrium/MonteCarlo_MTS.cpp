...
MonteCarlo_MTS::MonteCarlo_MTS(int my_id, int root_proc, int num_procs, int num_beads,
                               int elec_beads,double mass, int num_states,
...
void MonteCarlo_MTS::runSimulation(){
    ...
    if(readPSV){myHelper.read_PSV(num_beads, elec_beads, num_states, Q, x, p);}
    
    if (readData) {myHelper.read_MC_data(sgn_total, estimator_total);}
    ...
    decorrelation myDecorr(num_beads,elec_beads,num_steps);
    bool calc_decorr = true;
    ...
    if(writePSV) {myHelper.write_PSV(num_beads, elec_beads, num_states, Q, x, p);}
    if (writeData) {myHelper.write_MC_data(sgn_total, estimator_total);}
    if (calc_decorr) {myDecorr.write_all_centroids(root);}
...
void MonteCarlo_MTS::set_num_steps(unsigned long long num_steps_In){num_steps = num_steps_In;}

void MonteCarlo_MTS::set_esti_rate(unsigned long long esti_rate_In){esti_rate = esti_rate_In;}

void MonteCarlo_MTS::set_write_PSV(bool set_In){writePSV = set_In;}

void MonteCarlo_MTS::set_read_PSV(bool set_In){readPSV = set_In;}

void MonteCarlo_MTS::set_read_Data(bool set_In){readData = set_In;}

void MonteCarlo_MTS::set_write_Data(bool set_In){writeData = set_In;}
